...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
  ...
  func checkLoginStatus() {
    guard let checkLogedIn = UserDefaultsManager.shared.boolValue(forKey: "isLoggedIn") else { return }
    if checkLogedIn {
      setToRootViewController(ViewController())
    } else {
      setToRootViewController(AuthenticationViewController())
    }
  }
  
  func setToRootViewController(_ vc: UIViewController) {
  ...
  private func handleURL(_ url: URL) {
    ...
    if let hostType = HostType(rawValue: host) {
